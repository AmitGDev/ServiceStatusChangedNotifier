...
#include <syncstream> // For: std::osyncstream (Since C++20)
...
namespace  // (Anonymous namespace)
{
	...
	void OnNotificationActionFunction(const std::wstring& service_name, const DWORD& current_state)
	{
		...
		sync_stream << L"notification: " << service_name << L" current state: " << current_state << std::endl;

		if (current_state == SERVICE_NOTIFY_STOPPED) { // Only if service is stopped:
			sync_stream << L"action" << std::endl;
		}
	}
...
int main()
{
	...
	// Exit (unsubscibe all):
	service_status_change_notifier.Stop(); // Test: Set BP on Sleep(). On break, Set-Next-Statement here + single-step (to see that the WT exited).
}